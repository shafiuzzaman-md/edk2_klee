...
#include "/home/mdshafiuzzaman/Research/ISSTA24/Experiments/edk2_klee/edk2-master/MdeModulePkg/Include/Protocol/LockBox.h"
#include "/home/mdshafiuzzaman/Research/ISSTA24/Experiments/edk2_klee/edk2-master/MdeModulePkg/Include/Guid/SmmLockBox.h"

BOOLEAN  mLocked = FALSE;

UINTN    VerificationSet = 0;
  ...
  )
{
  ...
  //
  // Changing to offset copy.
  //
  UINTN offset = 0;
  OffsetCopy(sizeof(EFI_SMM_LOCK_BOX_PARAMETER_SET_ATTRIBUTES), &offset, LockBoxParameterSetAttributes, &TempLockBoxParameterSetAttributes);
...
/**
  Verifies that SourceBuffer does not overlap with DestinationBuffer and returns result,
  without doing copy
...
/**
  ...
  This function copies Length bytes from SourceBuffer + Offset to DestinationBuffer, and returns
  DestinationBuffer.  The implementation must be reentrant, and it must handle the case
  where SourceBuffer overlaps DestinationBuffer.

  @param  Offset              A pointer to the offset value from source buffer.
  @param  DestinationBuffer   A pointer to the destination buffer of the memory copy.
  @param  SourceBuffer        A pointer to the source buffer of the memory copy.
  @param  Length              The number of bytes to copy from SourceBuffer to DestinationBuffer.
  ...
  )
{
  ...
  if (Offset == NULL || SourceBuffer == NULL ||
        DestinationBuffer == NULL || Length == 0) {
    return EFI_SUCCESS;
  }

  VOID *Source = (VOID *)(*Offset + (UINTN)SourceBuffer);

  if (VerificationSet == 3 )
    return VerifyNoDestSrcOverlap(Length, Source, DestinationBuffer);
  if (VerificationSet > 1 )
    return EFI_UNSUPPORTED;

  if (Length && (const void *)DestinationBuffer != Source )
    DuplicateMemory((void *)DestinationBuffer, Source, Length);
